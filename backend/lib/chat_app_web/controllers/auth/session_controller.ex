defmodule ChatAppWeb.Auth.SessionController do
  ...
  def login(conn, %{"user" => %{"login" => login, "password" => password}}) do
    ...
    with {:ok, user} <- Accounts.authenticate_user_by_login(login, password),
         ...
         {:ok, refresh, _} <- Guardian.encode_and_sign(user, %{}, token_type: "refresh", ttl: {1, :day}) do
      conn
  ...
  def refresh(conn, _params) do
    ...
    with refresh_token when is_binary(refresh_token) <- conn.cookies["ruid"],
         ...
         "refresh" <- claims["typ"],
         {:ok, access, _} <- Guardian.encode_and_sign(user, %{}, token_type: "access", ttl: {15, :minute}),
         {:ok, new_refresh, _} <- Guardian.encode_and_sign(user, %{}, token_type: "refresh", ttl: {1, :day}) do
      conn
